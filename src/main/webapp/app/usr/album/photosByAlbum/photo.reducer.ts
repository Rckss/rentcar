...
const apiUrl = 'api/photosbyalbum';
...
export const getEntitiesById: ICrudGetAllActionById<IPhoto> = (id, page, size, sort) => {
  const requestUrl = `${apiUrl}/${id}${sort ? `?page=${page}&size=${size}&sort=${sort}` : ''}`;
  return {
    type: ACTION_TYPES.FETCH_PHOTO_LIST,
    payload: axios.get<IPhoto>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
