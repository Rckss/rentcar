...
import { getEntities } from 'app/usr/photo/photo.reducer';
...
export const saveAccountSettings = account => async dispatch => {
  ...
  dispatch(getSession());
  // need id from user photo :(
  // dispatch get photo of specific user
  // dispatch(getEntity());
};

export const fetchUserAndPhoto = () => async dispatch => {
  await dispatch(getSession()).then(
    // need id from user photo :(
    // dispatch get photo of specific user
    // from now only getting the hole list of pictures, need to change to get a single picture.
